 ...
 * 
 * $Id: HibernateDao.java,v 1.1 2011/03/28 08:01:25 zwm Exp $
 */
...
import com.rosy.bill.dao.PropertyFilter.MatchType;
 ...
 * @author calvin
 */
public class HibernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK> implements Dao<T,PK>{
	/**
	 * 用于Dao层子类的构造函数.
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Page<T> findPageBySql(final Page<T> page, final String sql, final Object[] values, Type... types) {
		Assert.notNull(page, "page不能为空");

		Query q = createSqlQuery(sql, values, types);

		if (page.isAutoCount()) {
			long totalCount = countSqlResult(sql, values, types);
			page.setTotalCount(totalCount);
		}
	...
	/**
	 * 执行count查询获得本次Sql查询所能获得的记录总数.
	 * 
	 * 本函数只能自动处理简单的Sql语句,复杂的Sql查询请另行编写count语句查询.
	 */
	protected long countSqlResult(final String sql, final Object[] values, Type... types) {
		String countSql = prepareCountSql(sql);

		try {
			Long count = ((java.math.BigDecimal)findUniqueBySql(countSql, values, types)).longValue();
			return count;
		} catch (Exception e) {
	...
	protected long countSqlResult(final String sql, final Map<String, ?> values) {
		...
		try {
			Long count = ((java.math.BigDecimal)findUniqueBySql(countSql, values)).longValue();
			return count;
		} catch (Exception e) {
			throw new RuntimeException("hql can't be auto count, hql is:" + countSql, e);
		}
